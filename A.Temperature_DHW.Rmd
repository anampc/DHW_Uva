...
#write_csv(Uva.OI.MonthlyClimatology_1985_2012, "Outputs/UvaSST.M_clima_mean.csv")
```
...
#write_csv(InSituMonthlyClimatology, "Outputs/UvaInSitu.M_clima_mean.csv")
```
...
##### Calculate IO DHW
      ...
      #write.csv(Uva.InSitu.L, "Outputs/GC_DHW_Daily_Uva.csv")
```
...
Figure1b_Final<-grid.arrange(TemperaturePlot, DHWPlot, nrow=2, heights=c(6/10, 4/10))

ggsave(file="C.Outputs/Figure_1b.svg", plot=Figure1b_Final, width=6.5, height=4.5)

```

**Figure 1**: Accumulated heat stress and thermal regime at Uva Island, Gulf of Chiriquí. **b.** Thermal regime before and during the 2015-16 El Niño using daily OISST (black) and in situ (gray) data collected at Uva Island reef (~3m depth). Upper plot and right-hand axis show mean daily temperature (solid lines) and bleaching threshold (dashed lines) for each data set. Lower plot and and left-hand axis show DHW calculated from each data source. The blue vertical lines represent the periods when the coral samples were collected. </br> 
...
# 7. OI DHW for Banderas Bay (Supplementary Information)
...
#OI_SST 
    Mex.OI<-read.csv("B.Temperature_data/BanderasBay_Daily_OISST_1982-2016.csv",header = T)
    Mex.OI$Date<-as.Date(Mex.OI$Date, format="%Y-%m-%d")

```
 
Calculate MMM: For the climatology and maximum monthly mean (MMM) we selected the SST data from 1985-2012, to keep consistency with the CRW methodology

```{r Climatology_mex}
  Mex.OI$Month<-month(Mex.OI$Date)
  Mex.OI$Year<-year(Mex.OI$Date)
  Mex.OI$Location<-as.factor(Mex.OI$Location) 
  
  # SELECT YEARS 1985-2012
    M_SST.1985_2012<-filter(Mex.OI, Year>1984)
    M_SST.1985_2012<-filter(M_SST.1985_2012, Year<2013)
  
  # Mean temperature per month in each location
  M_OI.MonthlyClimatology_1985_2012<-M_SST.1985_2012 %>%
    group_by(Location, Month) %>%
    summarize(Clima_M = mean(OISST_mex, na.rm = TRUE))
  #OI.MonthlyClimatology_1985_2012
  # Maximum Monthly Mean in each location
  M_MMM_OI<-M_OI.MonthlyClimatology_1985_2012 %>%
    group_by(Location) %>%
...
```{r HotSpot_mex}
Mex.OI <- merge(Mex.OI, M_MMM_OI, on="Site") # Add the MMM to each location
Mex.OI <- Mex.OI[order(Mex.OI$Location, Mex.OI$Date),]
Mex.OI$HotSpot<-(Mex.OI$OISST_mex-(Mex.OI$MMM))
Mex.OI$HotSpot<-ifelse(Mex.OI$HotSpot>=0,
                       Mex.OI$HotSpot, 0)# REMOVE NEGATIVE ANOMALIES
Mex.OI$D_Stress<-ifelse(Mex.OI$HotSpot>=1,
                       Mex.OI$HotSpot, 0) # REMOVE HotSpots <1
Mex.OI$W_Stress<-(Mex.OI$D_Stress/7) # Transform daily to weekly values
```
...
```{r Mexico_summary}
# Summary by Site and year (max values)
  DHW_Year_Summary<-DHW_Daily %>%
         group_by(Location, Year) %>%
         dplyr::summarise(maxDHW = max(NOAA_DHW_OI),
                          maxSST = max(OISST_mex))

# Select major ENSO years
Table_S1<- DHW_Year_Summary[ which(
  DHW_Year_Summary$Year==1982 |
  DHW_Year_Summary$Year==1983 |
  DHW_Year_Summary$Year==1997 |
  DHW_Year_Summary$Year==1998 |
  DHW_Year_Summary$Year==2015 |
  DHW_Year_Summary$Year==2016), ]

#Wide format data
    Table_S1<-Table_S1[, -4]
    Table_S1 <-spread(Table_S1, Year, maxDHW)
```

### Table S1

```{r TableS1, echo=FALSE}
kable(Table_S1, caption = "Supplementary Table S1: Maximum annual DHW experienced at Banderas Bay (Mexico), an ETP location not affected by the 1982-83 ENSO, but that was heavily impacted by the 1997-98 event. Uva Island values have been added to facilitate comparison.")
```
